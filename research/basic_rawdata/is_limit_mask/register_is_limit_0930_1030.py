#!/usr/bin/env python3
"""Register upper/lower limit masks for the 09:30-10:30 execution window.

Definitions:
  is_upper_limit_0930_1030 - 1.0 if >=60% of 1m bars at upper limit, else 0.0
  is_lower_limit_0930_1030 - 1.0 if >=60% of 1m bars at lower limit, else 0.0
...
DEFINITIONS = [
    AShareRawDataDefinition(
        ...
        description="涨停检测：09:30-10:30 时段内 >=60% 的 1m close 在涨停价 0.05% 以内",
    ),
    AShareRawDataDefinition(
        name="is_lower_limit_0930_1030",
        formula=_LOWER_LIMIT_FORMULA,
        func_name="apply_func",
        input_names=["origin_close"],
        daily_input_names=["low_limited"],
        output_names=["is_lower_limit_0930_1030"],
        params=RawDataParams(input_time_filter=[("09:30", "10:30")]),
        slot=RawDataSlot.MIDDAY,
        data_available_at=1030,
        execution_start_at=930,
        ...
        expected_bars=60,
        price_basis=PriceBasis.ORIGIN,
        description="跌停检测：09:30-10:30 时段内 >=60% 的 1m close 在跌停价 0.05% 以内",
    ),
]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build or register upper/lower limit masks for 09:30-10:30",
    )
    parser.add_argument(
        "--register",
        action="store_true",
